...
button::button(const scaled& scaled, const sf::Texture& active, sf::Text&& text):
	_scaled(scaled), _active(active), _inactive(*scaled.getSprite().getTexture()), _state(false), _text(std::move(text)) { _text.setPosition(scaled.getPosition().x+15, scaled.getPosition().y+10); _text.setCharacterSize(game::window::defaultFontSize*game::window::factors.x);}

button::button(scaled&& scaled, const sf::Texture& active, sf::Text&& text):
	_scaled(std::move(scaled)), _active(active), _inactive(*_scaled.getSprite().getTexture()), _state(false), _text(std::move(text)) { _text.setPosition(scaled.getPosition().x+15, scaled.getPosition().y+10); _text.setCharacterSize(game::window::defaultFontSize*game::window::factors.x);}
